...
const TrainRegisWait = () => {
      ...
      try {
        ...
        if (Array.isArray(data)) {
          ...
          const approved = data.filter(
            (course: Training) => course.status === "รออนุมัติ"
          );
  ...
  return (
    <div className="TrainRegisWait-warrapper">
      <NavbarTrader />

      <div className="TrainRegisWait-container">
        <div className="TrainRegisWait-container-search-input">
          <i className="bi bi-search"></i>
          <input type="text" placeholder="ค้นหาหลักสูตรอบรม" />
        </div>
        <div className="TrainRegisWait-container-link">
      ...
      <div className="TrainRegisWait-container-card">
        {isLoading ? (
          <p>กำลังโหลด...</p>
        ) : approvedCourses.length === 0 ? (
          <p>ยังไม่มีหลักสูตรที่รออนุมัติ</p>
        ) : approvedCourses.filter(
            (training) => training.statusCourse === "รอยืนยันการผ่านอบรม"
          ).length === 0 ? (
          // ถ้าไม่มีหลักสูตรที่มีสถานะ "รอยืนยันการผ่านอบรม"
          <p>ยังไม่มีหลักสูตรที่รออนุมัติและหลักสูตรที่รอยืนยันการผ่านอบรม</p>
        ) : (
